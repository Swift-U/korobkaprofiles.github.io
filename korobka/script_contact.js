...
function setup_compy_form() {
  ...
  dropdown_inputs.filter(".focus-helper").blur(function() {
    $(this)
      .closest(".dropdown")
      .removeClass("open");
  });
  ...
  radio_inputs.filter("[checked]").each(function() {
  ...
  function set_input_label_as_valid(label) {
    if (label.hasClass("invalid")) {
      ...
      label.removeClass("invalid").text(label.data("text"));
    }
  }
  ...
  function clear_all() {
    underlines.filter(".float-label").removeClass("float-label");
    ...
    checkbox_inputs.filter(":checked").prop("checked", false);
